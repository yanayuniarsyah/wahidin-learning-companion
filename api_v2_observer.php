...
if (!isset($user['role']) || $user['role'] === 'siswa') {
    http_response_code(403);
    echo json_encode(['error' => 'Forbidden: Observers only']);
    exit;
}
...
if (!$observer_id) {
    ...
    echo json_encode(['error' => 'Unauthorized: Invalid observer identity']);
    exit;
}

// Helper functions for session ownership
function getSession($db, $session_id, $observer_id) {
    $stmt = $db->prepare("SELECT * FROM wlc_sessions WHERE id = ? AND observer_id = ?");
    $stmt->execute([$session_id, $observer_id]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

// ROUTING
if ($uri === '/api/v2/observer/session/active' && $method === 'GET') {
    ...
    $stmt = $db->prepare("SELECT * FROM wlc_sessions WHERE siswa_id = ? AND observer_id = ? AND UPPER(status) = 'DRAFT' ORDER BY id DESC LIMIT 1");
...
if ($uri === '/api/v2/observer/session' && $method === 'POST') {
    $siswa_id = $inputBody['siswa_id'] ?? null;
    if (!$siswa_id) {
        http_response_code(400);
        echo json_encode(['error' => 'siswa_id required']);
        exit;
    }
    $methodology = $inputBody['methodology'] ?? 'STRUCTURED OBSERVATION';
    ...
    $stmt = $db->prepare("SELECT id FROM wlc_sessions WHERE siswa_id = ? AND observer_id = ? AND instrument_id = ? AND UPPER(status) = 'DRAFT' ORDER BY id DESC LIMIT 1");
    $stmt->execute([$siswa_id, $observer_id, $instrument_id]);
    ...
    $stmt = $db->prepare("INSERT INTO wlc_sessions (siswa_id, observer_id, respondent_type, instrument_id, status) VALUES (?, ?, 'OBSERVER', ?, 'DRAFT')");
    $stmt->execute([$siswa_id, $observer_id, $instrument_id]);
